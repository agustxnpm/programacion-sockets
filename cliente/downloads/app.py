...
class ChatApp:
    def __init__(self, root: ctk.CTk):
        ...
        self._file_recv: FileReceiver | None = None
    ...
    def _handle_incoming_file_notice(self, payload: bytes):
        ...
        def on_accept():
            self._net.send(protocol.build_ack(0x02, bytes([0x01])))
            self._file_recv = FileReceiver(filename, size, DOWNLOAD_DIR)
            if size >= 1024 * 1024:
                size_str = f"{size / (1024 * 1024):.2f} MB"
            else:
                size_str = f"{size / 1024:.1f} KB"
            self._chat_view.show_system(f"Recibiendo '{filename}' ({size_str})...")

        def on_reject():
            self._net.send(protocol.build_ack(0x02, bytes([0x00])))
            self._chat_view.show_system(f"Archivo '{filename}' rechazado.")
    ...
    def _handle_incoming_chunk(self, payload: bytes):
        ...
        self._file_recv.write_chunk(chunk_data)
        self._net.send(protocol.build_ack(0x01))

        if self._file_recv.is_complete:
            self._file_recv.close()
            self._chat_view.show_system(
                f"'{self._file_recv.filename}' guardado en '{DOWNLOAD_DIR}/'."
            )
